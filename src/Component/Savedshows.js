...
const Savedshows = () => {
    ...
    const slideleft = ()=>{
      ...
      useEffect(() => {
        ...
        // Cleanup function to stop listening for changes to the document
        return () => unsubscribe();
      }, [user?.email]);
      const movieRef = doc(db,'users' ,`${user?.email}`)
      const deleteShow = async(passedID)=>{
        try{
           const result = movies.filter((it)=>it.id!==passedID)
      ...
      <div className='relative flex items-center group'>
        ...
        <div id={'slider'} className='w-full h-full overflow-x-scroll whitespace-nowrap scroll-smooth scrollbar-hide relative'>
          {movies.map((it, id) => (
            ...
            <img className='w-full h-auto block' src={`https://image.tmdb.org/t/p/w500/${it?.img}`} alt={it?.title} />
            <div className='absolute top-0 left-0 w-full h-full hover:bg-black/80 opacity-0 hover:opacity-100 text-white'>
                ...
                <p onClick={()=> deleteShow(it.id)} className='absolute text-gray-300 top-4 right-4' ><AiOutlineClose/></p>
